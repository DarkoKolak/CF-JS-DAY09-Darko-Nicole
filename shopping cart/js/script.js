...
function add(i){
 ...
 document.getElementById("result").innerHTML += `<div class="innerboxes2"><img src="${cars[i].image}"><p>${cars[i].name}</p><p>Production Year: ${cars[i].productionYear}</p><p>Price: ${cars[i].price}</p><button id="${i}" class = "fromcart">Remove from Cart</button></div>`;

 var firstbtn = document.getElementsByClassName("fromcart");

 		 for (var i =0;  i< cars.length; i++) {
	firstbtn.addEventListener("click", function(){remove(this.getAttribute("id"))},false);
}
